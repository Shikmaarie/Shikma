        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          <Reveal delay={0.08}>
            <SectionTitle className="mt-6">
              להתחיל <span className="text-teal">בלי לשלם שקל</span>
            </SectionTitle>
          </Reveal>
        </div>
        ...
        <div className="mx-auto mt-14 grid max-w-4xl gap-6 md:grid-cols-2">
          {freeTrainings.map((training, i) => (
            <Reveal key={training.title} delay={0.08 + i * 0.09} className="h-full">
              <article className="flex h-full flex-col rounded-4xl panel p-8">
                <BookOpen className="size-6 text-accent" aria-hidden="true" />

                <h3 className="mt-5 font-display text-xl font-bold text-fg">
                  {training.title}
                </h3>
                <p className="mt-3 flex-1 leading-relaxed text-fg2">
                  {training.body}
                </p>

                <p className="mt-5 font-display text-2xl font-black text-teal">
                  חינם
                </p>
                ...
                <a
                  ...
                  className="mt-5 inline-flex items-center justify-center rounded-full border border-line-strong px-6 py-3 text-sm font-bold text-fg transition hover:border-gold/80 hover:text-accent"
                >
                  {training.cta}
                </a>
              </article>
